...
void print (vector<vector<int> *> runs, int pageSize){
    int loop = runs.size();
...
void mergeKSorted(vector<vector<int> *> runs, vector<int> *rec, int k, int start, int pageSize)
{
    if (k <= 0)
        return;
...
void MergeRuns(vector<vector<int> *> &runs, int B, int pageSize)
{
    // print (runs, pageSize);
    // cout << endl;
    // cout << endl;
    ...
    {
        int offset = i * (B - 1);
        mergeKSorted(runs, rec, B - 1, offset, pageSize);
    ...
    {
        int offset = h * (B - 1);
        mergeKSorted(runs, rec, h1, offset, pageSize);

        run2.push_back(rec);
    }

    runs = run2;
    MergeRuns(runs, B, pageSize);
